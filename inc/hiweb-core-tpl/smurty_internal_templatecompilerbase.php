 ...
 * @subpackage Compiler
 */
abstract class Smurty_Internal_TemplateCompilerBase
{
    /**
    ...
    /**
     * suppress hiweb_tpl header code in compiled template
     *
     * @var bool
     */
     ...
     * It executes the required compile plugin for the hiweb_tpl tag
     *
    ...
    public function compileTag($tag, $args, $parameter = array())
    {
        ...
        // log tag/attributes
        if (isset($this->smurty->get_used_tags) && $this->smurty->get_used_tags) {
        ...
        // compile the smurty tag (required compile classes to compile the tag are autoloaded)
        if (($_output = $this->callTagCompiler($tag, $args, $parameter)) === false) {
            if (isset($this->smurty->template_functions[$tag])) {
        ...
        if ($_output !== false) {
            ...
            if (strlen($tag) < 6 || substr($tag, - 5) != 'close') {
                ...
                if (isset($this->smurty->registered_objects[$tag]) && isset($parameter['object_method'])) {
                    $method = $parameter['object_method'];
                    if (!in_array($method, $this->smurty->registered_objects[$tag][3]) &&
                        (empty($this->smurty->registered_objects[$tag][1]) || in_array($method, $this->smurty->registered_objects[$tag][1]))
                    ) {
                        return $this->callTagCompiler('private_object_function', $args, $parameter, $tag, $method);
                    } elseif (in_array($method, $this->smurty->registered_objects[$tag][3])) {
                ...
                foreach (array(hiweb_tpl::PLUGIN_COMPILER, hiweb_tpl::PLUGIN_FUNCTION, hiweb_tpl::PLUGIN_BLOCK) as $plugin_type) {
                    if (isset($this->smurty->registered_plugins[$plugin_type][$tag])) {
                        // if compiler function plugin call it now
                        if ($plugin_type == hiweb_tpl::PLUGIN_COMPILER) {
                            $new_args = array();
                            ...
                            if (!$this->smurty->registered_plugins[$plugin_type][$tag][1]) {
                                $this->tag_nocache = true;
                            }
                            $function = $this->smurty->registered_plugins[$plugin_type][$tag][0];
                            if (!is_array($function)) {
                                return $function($new_args, $this);
                            } elseif (is_object($function[0])) {
                                return $this->smurty->registered_plugins[$plugin_type][$tag][0][0]->$function[1]($new_args, $this);
                            } else {
                                return call_user_func_array($function, array($new_args, $this));
                            }
                        }
                        // compile registered function or block function
                        if ($plugin_type == hiweb_tpl::PLUGIN_FUNCTION || $plugin_type == hiweb_tpl::PLUGIN_BLOCK) {
                            return $this->callTagCompiler('private_registered_' . $plugin_type, $args, $parameter, $tag);
                        }
                    }
                }
                // check plugins from plugins folder
                foreach ($this->smurty->plugin_search_order as $plugin_type) {
                    if ($plugin_type == hiweb_tpl::PLUGIN_COMPILER && $this->smurty->loadPlugin('smurty_compiler_' . $tag) && (!isset($this->smurty->security_policy) || $this->smurty->security_policy->isTrustedTag($tag, $this))) {
                        $plugin = 'smurty_compiler_' . $tag;
                        if (is_callable($plugin)) {
                            ...
                            return $plugin($new_args, $this->smurty);
                        }
                        ...
                        if ($function = $this->getPlugin($tag, $plugin_type)) {
                            if (!isset($this->smurty->security_policy) || $this->smurty->security_policy->isTrustedTag($tag, $this)) {
                                return $this->callTagCompiler('private_' . $plugin_type . '_plugin', $args, $parameter, $tag, $function);
                            }
                        }
                    }
                }
                if (is_callable($this->smurty->default_plugin_handler_func)) {
                    $found = false;
                ...
                if (isset($this->smurty->registered_objects[$base_tag]) && isset($parameter['object_method'])) {
                    $method = $parameter['object_method'];
                    if (in_array($method, $this->smurty->registered_objects[$base_tag][3])) {
                        return $this->callTagCompiler('private_object_block_function', $args, $parameter, $tag, $method);
                    } else {
                        // throw exception
                ...
                if (isset($this->smurty->registered_plugins[hiweb_tpl::PLUGIN_BLOCK][$base_tag]) || isset($this->default_handler_plugins[hiweb_tpl::PLUGIN_BLOCK][$base_tag])) {
                    return $this->callTagCompiler('private_registered_block', $args, $parameter, $tag);
                }
                // block plugin?
                if ($function = $this->getPlugin($base_tag, hiweb_tpl::PLUGIN_BLOCK)) {
                    return $this->callTagCompiler('private_block_plugin', $args, $parameter, $tag, $function);
                }
                // registered compiler plugin ?
                if (isset($this->smurty->registered_plugins[hiweb_tpl::PLUGIN_COMPILER][$tag])) {
                    // if compiler function plugin call it now
                    $args = array();
                    if (!$this->smurty->registered_plugins[hiweb_tpl::PLUGIN_COMPILER][$tag][1]) {
                        $this->tag_nocache = true;
                    }
                    $function = $this->smurty->registered_plugins[hiweb_tpl::PLUGIN_COMPILER][$tag][0];
                    if (!is_array($function)) {
                        return $function($args, $this);
                    } elseif (is_object($function[0])) {
                        return $this->smurty->registered_plugins[hiweb_tpl::PLUGIN_COMPILER][$tag][0][0]->$function[1]($args, $this);
                    } else {
                        return call_user_func_array($function, array($args, $this));
                    }
                }
                if ($this->smurty->loadPlugin('smurty_compiler_' . $tag)) {
                    $plugin = 'smurty_compiler_' . $tag;
                    if (is_callable($plugin)) {
                        return $plugin($args, $this->smurty);
                    }
                    if (class_exists($plugin, false)) {
                        $plugin_object = new $plugin;
                        if (method_exists($plugin_object, 'compile')) {
     ...
     * class name format:  Smurty_Internal_Compile_TagName
     * plugin filename format: Smurty_Internal_Tagname.php
     *
     * @param  string $tag    tag name
    ...
    public function callTagCompiler($tag, $args, $param1 = null, $param2 = null, $param3 = null)
    {
        ...
        $class_name = 'Smurty_Internal_Compile_' . $tag;
        if ($this->smurty->loadPlugin($class_name)) {
    ...
    public function getPlugin($plugin_name, $plugin_type)
    {
        ...
        $function = 'smurty_' . $plugin_type . '_' . $plugin_name;
        $file = $this->smurty->loadPlugin($function, false);
    ...
    public function getPluginFromDefaultHandler($tag, $plugin_type)
    {
        ...
        $result = call_user_func_array(
            $this->smurty->default_plugin_handler_func, array($tag, $plugin_type, $this->template, &$callback, &$script, &$cacheable)
        );
        if ($result) {
    ...
    public function processNocacheCode($content, $is_code)
    {
        ...
        if ($is_code && !empty($content)) {
            ...
            if ((!($this->template->source->recompiled) || $this->forceNocache) && $this->template->caching && !$this->suppressNocacheProcessing &&
                ...
                $_output = "<?php echo '/*%%SmurtyNocache:{$this->nocache_hash}%%*/" . $_output . "/*/%%SmurtyNocache:{$this->nocache_hash}%%*/';?>\n";
    ...
    {
        if ($this->smurty->debugging && $debug) {
            Smurty_Internal_Debug::end_compile($this->template);
        }
        array_push($this->trace_stack, array($this->smurty->_current_file, $this->trace_filepath, $this->trace_uid, $this->trace_line_offset));
        $this->trace_filepath = $this->smurty->_current_file = $file;
        $this->trace_uid = $uid;
     ...
     *
     * @throws SmurtyCompilerException when an unexpected token is found
     */
    public function trigger_template_error($args = null, $line = null)
    {
        ...
        $e = new SmurtyCompilerException($error_text);
        $e->line = $line;
