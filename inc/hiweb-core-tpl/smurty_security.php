...
/*
 * FIXME: Smurty_Security API
 *      - getter and setter instead of public properties would allow cultivating an internal cache properly
 *      - current implementation of isTrustedResourceDir() assumes that hiweb_tpl::$template_dir and hiweb_tpl::$config_dir are immutable
 *        the cache is killed every time either of the variables change. That means that two distinct hiweb_tpl objects with differing
 *        $template_dir or $config_dir should NOT share the same Smurty_Security instance,
 ...
 */
class Smurty_Security
{
    /**
     * This determines how hiweb_tpl handles "<?php ... ?>" tags in templates.
     * possible values:
     * <ul>
     *   <li>hiweb_tpl::PHP_PASSTHRU -> echo PHP tags as they are</li>
     *   <li>hiweb_tpl::PHP_QUOTE    -> escape tags as entities</li>
     *   <li>hiweb_tpl::PHP_REMOVE   -> remove php tags</li>
     *   <li>hiweb_tpl::PHP_ALLOW    -> execute php tags</li>
     * </ul>
     *
     * @var integer
     */
    public $php_handling = hiweb_tpl::PHP_PASSTHRU;
    /**
     ...
     * @throws SmurtyCompilerException if php function is not trusted
     */
     ...
     * @throws SmurtyCompilerException if static class is not trusted
     */
     ...
     * @throws SmurtyCompilerException if modifier is not trusted
     */
     ...
     * @throws SmurtyException if stream is not trusted
     */
    public function isTrustedStream($stream_name)
    {
        ...
        throw new SmurtyException("stream '{$stream_name}' not allowed by security setting");
    }
     ...
     * @throws SmurtyException if directory is not trusted
     */
    public function isTrustedResourceDir($filepath)
    {
        ...
        $_template_dir = $this->smurty->getTemplateDir();
        $_config_dir = $this->smurty->getConfigDir();
        ...
        while (true) {
            ...
            if (($pos = strrpos($directory, DIR_SEPARATOR)) === false || !isset($directory[1])) {
                break;
            }
            // bubble up one level
        ...
        // give up
        throw new SmurtyException("directory '{$_filepath}' not allowed by security setting");
    }
     ...
     * @throws SmurtyException if URI is not trusted
    ...
    public function isTrustedUri($uri)
    {
        ...
        throw new SmurtyException("URI '{$uri}' not allowed by security setting");
    }
     ...
     *
     * @return boolean         true if directory is trusted
     * @throws SmurtyException if PHP directory is not trusted
     */
    public function isTrustedPHPDir($filepath)
    {
        if (empty($this->trusted_dir)) {
            throw new SmurtyException("directory '{$filepath}' not allowed by security setting (no trusted_dir specified)");
        }
        ...
        while (true) {
            ...
            // abort if we've reached root
            if (($pos = strrpos($directory, DIR_SEPARATOR)) === false || !isset($directory[2])) {
                break;
            }
            // bubble up one level
            $directory = substr($directory, 0, $pos);
