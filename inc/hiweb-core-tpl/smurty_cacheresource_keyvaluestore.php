...
/**
 * hiweb_tpl Cache Handler Base for Key/Value Storage Implementations
 ...
 * @author     Rodney Rehm
 */
abstract class Smurty_CacheResource_KeyValueStore extends Smurty_CacheResource
{
    /**
     * cache for contents
     *
    ...
    {
        $uid = $this->getTemplateUid($smurty, $resource_name, $cache_id, $compile_id);
     ...
     */
    protected function getTemplateUid(hiweb_tpl $smurty, $resource_name, $cache_id, $compile_id)
    {
        $uid = '';
    ...
    public function hasLock(hiweb_tpl $smurty, Smurty_Template_Cached $cached)
    {
        ...
        return $data && time() - $data[$key] < $smurty->locking_timeout;
    }
    ...
    public function acquireLock(hiweb_tpl $smurty, Smurty_Template_Cached $cached)
    {
        ...
        $key = 'LOCK#' . $cached->filepath;
        $this->write(array($key => time()), $smurty->locking_timeout);
    }
